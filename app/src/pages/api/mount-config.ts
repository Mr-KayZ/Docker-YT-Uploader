...
import { VIDEOS_DIR, AUTH_DIR, DATA_DIR } from "../../lib/paths.ts";
import { loadMountConfig, saveMountConfig } from "../../lib/mountConfig.ts";
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        ok: true,
        message:
          "Mount config saved. Restart the container for changes to take effect.",
        saved: config,
      }),
      { status: 200, headers: { "Content-Type": "application/json" } },
    );
